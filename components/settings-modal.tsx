...
function Toggle({ checked, onChange, label }: { checked: boolean; onChange: (v: boolean) => void; label: string }) {
  return (
    <button
      ...
      className={`relative h-7 w-12 shrink-0 rounded-full border transition-colors duration-300 ${checked ? "border-crimson-bright/60 bg-crimson/40" : "border-border bg-secondary"}`}
    >
  ...
  async function handleTest() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setTestMessage("Test notification sent to Discord.")
      } else {
        setTestState("error")
        setTestMessage(data.error ?? "Failed to send test webhook.")
      }
    } catch {
      setTestState("error")
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="settings-title">
      ...
      <div className="animate-modal-in glass-panel relative w-full max-w-md rounded-2xl p-6 shadow-[0_0_60px_-12px_var(--crimson)]">
        <div className="mb-6 flex items-center justify-between">
          ...
          <button onClick={onClose} aria-label="Close" className="flex h-9 w-9 items-center justify-center rounded-lg border border-border text-muted-foreground transition-colors hover:border-crimson-bright/50 hover:text-foreground">
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <div className="space-y-6">
          <div className="flex items-start justify-between gap-4">
            <div className="flex gap-3">
              <Volume2 className="mt-0.5 h-5 w-5 shrink-0 text-crimson-bright" />
              <div>
                <p className="font-mono text-xs font-semibold uppercase tracking-wider text-foreground">Status Changing Sounds</p>
                <p className="mt-1 text-sm leading-relaxed text-muted-foreground">Play a notification sound whenever the detected server status changes in this browser.</p>
              </div>
            </div>
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="flex gap-3">
              <Webhook className="mt-0.5 h-5 w-5 shrink-0 text-crimson-bright" />
              <div>
                <p className="font-mono text-xs font-semibold uppercase tracking-wider text-foreground">Webhook Test</p>
                <p className="mt-1 text-sm leading-relaxed text-muted-foreground">Automatic status notifications run server-side. This control is only for manually testing a Discord webhook.</p>
              </div>
            </div>
            <Toggle label="Show webhook test controls" checked={settings.webhookEnabled} onChange={(v) => onChange({ ...settings, webhookEnabled: v })} />
          </div>
          ...
          {settings.webhookEnabled && (
            <div className="animate-modal-in space-y-3">
              ...
              <button type="button" onClick={handleTest} disabled={!settings.webhookUrl || testState === "loading"} className="flex w-full items-center justify-center gap-2 rounded-lg border border-crimson/50 bg-crimson/15 px-4 py-3 font-mono text-xs font-semibold uppercase tracking-widest text-foreground transition-all hover:border-crimson-bright/70 hover:bg-crimson/25 disabled:cursor-not-allowed disabled:opacity-40">
                {testState === "loading" ? <Loader2 className="h-4 w-4 animate-spin" /> : testState === "success" ? <Check className="h-4 w-4 text-status-green" /> : testState === "error" ? <AlertTriangle className="h-4 w-4 text-status-yellow" /> : null}
                Test Webhook
              </button>
